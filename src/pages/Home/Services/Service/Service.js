...
const Service = ({ imageLink, serviceName, description, price, _id }) => {
    ...
    return (
        <div className="w-full sm:w-1/2 md:w-1/2 lg:w-1/4 px-4 py-10 bg-white mt-6  hover:drop-shadow-2xl  rounded-lg dark:bg-gray-800">
            ...
            <Link to={`/bookingService/${_id}`} className="  focus:ring-purple-500 focus:ring-offset-purple-200  w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2   uppercase my-2 py-2 px-4 rounded-lg bg-pink-500 border-2 border-transparent text-white text-md mr-4 hover:bg-pink-400 ">
                Book Service
            </Link>
        </div >
    );

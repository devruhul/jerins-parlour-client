...
const BookingsLists = () => {
  ...
  useEffect(() => {
    fetch(
      `https://jerins-parlour-backend.herokuapp.com/bookings/${parlourUser.email}`
    )
      .then((res) => res.json())
    ...
    return (
      <div className='text-center my-6'>
        <h2 className='mb-6 text-3xl font-bold text-center text-gray-800 dark:text-white'>
          There is no booking, please book a service and check back later
        </h2>
      </div>
    );
  }
      ...
      <div className=' mx-auto'>
        ...
        <div className='flex flex-wrap -m-4'>
          {bookingsList.map((bookingList) => (
            <BookingList key={bookingList._id} {...bookingList} />
          ))}
        </div>
      </div>
    </section>
